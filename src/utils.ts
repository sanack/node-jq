...
export function validIfType(valType: string, fn: Function) {
  return function (value: any) {
    if (typeof value === valType) return fn(value);
    throw new Error(INVALID_TYPE_ERROR);
  }
}

export function validIfTrue(fn: Function) {
  return validIfType("boolean", function (value: boolean) {
    if (value === true) return fn(value);
  });
};

export function validIfArrayOfType(valType: string, fn: Function) {
  return function (value: any) {
    if (!Array.isArray(value)) throw new Error(INVALID_TYPE_ERROR);

    const valTypes = new Set(value.map((x: any) => typeof x));
    if (valTypes.size !== 1) throw new Error(INVALID_TYPE_ERROR);

    validIfType(valType, function () {
      return true;
    })(value[0]);
